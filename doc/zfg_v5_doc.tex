...
\pgfqkeys{/VorlageVersion5}{
	...
	all/Untertitel 					= {Eine \LaTeX-Vorlage mit vielen vordefinierten Befehlen.\par Version~5.4},
	...
	\chapter{Eigene Befehle}
		...
		\section{Deaktivierung vorhandener Befehle}
			...
			\fbox{
				\begin{minipage}[t]{.9\textwidth}
					\texttt{\textbackslash qed   \textbackslash let \textbackslash qed \textbackslash relax 	\textbackslash qed}
				\begingroup				
					\qed \let\qed\relax \qed
				\endgroup
			\end{minipage}
			}
			\vspace*{1em}\\
			  Das zweite qed-Zeichen wird durch \verb|\relax| unterdrückt.
		...
		\section{Begriffsdefinitionen}
			...
			\begin{quotex}[\href{https://de.wikipedia.org/wiki/Betriebssystem}{Wikipedia}]
				Ein \neuerbegriff{Betriebssystem} ist eine Zusammenstellung von Computerprogrammen, die die Systemressourcen eines Computers [\ellipse] zur Verfügung stell1t. 
			\end{quotex}
			Der Befehl \verb|\neuerbegriffIdx{}| funktioniert wie folgt:
			\begin{itemize}
				\item Gibt man nur einen Parameter an, so erscheint der Text anstelle des Befehls und der gleiche Text steht im Index: \verb|\neuerbegriffIdx{Betriebssystem}| gibt \texttt{Betriebssystem} aus und schreibt auch \texttt{Betriebssystem} in den Index.
